use pinocchio::{AccountView, Address, ProgramResult, cpi::Seed, error::ProgramError,AssociatedTokenAccount};
use core::mem::size_of;
use crate::{Escrow, instructions::helpers};
...
impl<'a> TryFrom<&'a [AccountView]> for MakeAccounts<'a> {
    ...
    fn try_from(accounts: &'a [AccountView]) -> Result<Self, Self::Error> {
        ...
        helpers::validate_maker_account(maker)?;
        helpers::validate_mint_account(mint_a)?;
        helpers::validate_mint_interface(mint_b)?;
        helpers::validate_associated_token_account(maker_ata_a, maker, mint_a, token_program)?;
        helpers::validate_system_program(system_program)?;
        helpers::validate_token_program(token_program)?;
...
impl<'a> TryFrom<(&'a [u8], &'a [AccountView])> for Make<'a> {
  ...
  fn try_from((data, accounts): (&'a [u8], &'a [AccountView])) -> Result<Self, Self::Error> {
    ...
    let (_, bump) = Address::find_program_address(&[b"escrow", accounts.maker.address().as_ref(), &instruction_data.seed.to_le_bytes()], &crate::ID);
